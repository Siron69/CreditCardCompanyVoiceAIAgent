...
var report_suspicious_transaction_default = import_schema.w.tool({
  ...
  params: import_schema.s.object({
    transaction_id: import_schema.s.string().describe("ID of the transaction to dispute"),
    reason: import_schema.s.string().describe("Reason: 'unrecognised', 'wrong_amount', 'goods_not_received', 'fraud', 'other'")
  }),
...
// dist/__report_suspicious_transaction_wrapper.ts
async function main(ctx, params) {
  const toolParams = ctx?.data ?? params ?? {};
  return await report_suspicious_transaction_default.handler(ctx, toolParams);
}
