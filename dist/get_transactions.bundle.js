...
var get_transactions_default = import_schema.w.tool({
  ...
  params: import_schema.s.object({
    limit: import_schema.s.optional(import_schema.s.number()).describe("Maximum number of transactions to return (default: 10)"),
    status_filter: import_schema.s.optional(import_schema.s.string()).describe("Filter by status: completed, pending, disputed, fraud")
  }),
...
// dist/__get_transactions_wrapper.ts
async function main(ctx, params) {
  const toolParams = ctx?.data ?? params ?? {};
  return await get_transactions_default.handler(ctx, toolParams);
}
