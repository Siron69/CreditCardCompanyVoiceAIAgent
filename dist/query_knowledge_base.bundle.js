...
var query_knowledge_base_default = import_schema.w.tool({
  ...
  params: import_schema.s.object({
    query: import_schema.s.string().describe("The question or topic to search for in the knowledge base")
  }),
...
// dist/__query_knowledge_base_wrapper.ts
async function main(ctx, params) {
  const toolParams = ctx?.data ?? params ?? {};
  return await query_knowledge_base_default.handler(ctx, toolParams);
}
