...
var redeem_rewards_default = import_schema.w.tool({
  name: "redeem-rewards",
  ...
  params: import_schema.s.object({
    reward_id: import_schema.s.string().describe("ID of the reward to redeem"),
    reward_name: import_schema.s.string().describe("Name of the reward (used for voice confirmation)"),
    confirmed: import_schema.s.boolean().describe("true if the customer has already confirmed, false to prompt for confirmation")
  }),
...
// dist/__redeem_rewards_wrapper.ts
async function main(ctx, params) {
  const toolParams = ctx?.data ?? params ?? {};
  return await redeem_rewards_default.handler(ctx, toolParams);
}
