...
var block_card_default = import_schema.w.tool({
  name: "block-card",
  description: "Immediately blocks the authenticated customer's card. Sends an SMS confirmation.",
  params: import_schema.s.object({
    reason: import_schema.s.string().describe("Reason for blocking: 'lost', 'stolen', 'suspicious_transactions', 'other'")
  }),
...
// dist/__block_card_wrapper.ts
async function main(ctx, params) {
  const toolParams = ctx?.data ?? params ?? {};
  return await block_card_default.handler(ctx, toolParams);
}
