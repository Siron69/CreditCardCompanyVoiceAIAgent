...
| Autenticazione | Nome + cognome + ultime 4 cifre carta, stato di sessione in KV, blocco a 3 tentativi | v1 usava il codice fiscale al posto del nome, ma dettare 16 caratteri alfanumerici a voce era il maggior rischio STT dell'intero agente (guidava l'auth success rate). Nome e cognome sono molto più robusti in trascrizione. Trade-off dichiarato: livello di sicurezza più basso, mitigato dal fatto che le azioni sensibili (sblocco) passano comunque da review umana; evoluzione pianificata: step-up con OTP via SMS. Il KV è per-conversazione, quindi l'autenticazione non trapela mai tra una chiamata e l'altra |
...
| Flusso di sblocco | L'agente crea il caso via API function `createUnblockCase` (con `interaction_id` e `interaction_channel` per la tracciabilità caso → conversazione); legge lo stato via `getUnblockCaseStatus` (per id) oppure via **SDK `ctx.tables.filter`** (caso più recente del cliente autenticato) | Copre il requisito "use both API functions and SDK table functions" con una ragione vera per ciascuno: la scrittura passa da un'unica function validata condivisa con l'app; la lettura per cliente usa l'SDK per evitare un hop HTTP in un turno vocale |
| Step-up verification sullo sblocco | Prima di inviare la richiesta di sblocco, il cliente deve riconfermare il **codice fiscale** (verificato contro la tabella `customers`, max 2 tentativi poi escalation), anche se già autenticato | Autenticazione proporzionata al rischio: le operazioni di consultazione usano la verifica leggera (nome + 4 cifre), l'unica operazione di sicurezza richiede un fattore aggiuntivo. Il codice fiscale qui è accettabile a voce perché il flusso è raro e ad alto valore — il costo UX dello spelling è giustificato |
...
- Il collegamento agente ↔ skill avviene nel tenant: il primo deploy dell'agente è fallito con
  ...
  volta dalla dashboard. Per i deploy successivi del solo prompt agente si usa
  `wonderful deploy agents ... --skip-deps`, che salta il sync delle skill (già agganciate).
